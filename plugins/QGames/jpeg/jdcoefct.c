...
#endif
#ifdef BLOCK_SMOOTHING_SUPPORTED
LOCAL(jp_bool) smoothing_ok JPP((j_decompress_ptr cinfo));
...
static jp_bool smoothing_ok (j_decompress_ptr cinfo)
{
  my_coef_ptr coef = (my_coef_ptr) cinfo->coef;
  jp_bool smoothing_useful = FALSE;
  int ci, coefi;
  jpeg_component_info *compptr;
  JQUANT_TBL * qtable;
  int * coef_bits;
...
void jinit_d_coef_controller (j_decompress_ptr cinfo, jp_bool need_full_buffer)
{
  my_coef_ptr coef;
